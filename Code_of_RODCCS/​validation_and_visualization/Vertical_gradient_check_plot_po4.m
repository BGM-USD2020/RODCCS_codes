...
load MGV.mat;
MM = MGV;
value_name = {'PhosphateC','POC','NH4','Temp','Salt','O2','Si','NO3','NO2','PO4','Chla','DIC'};
...
% ylabel('Depth (km)');
xlim([-0.1 2.5])
ylim([-0.02 max(odepth)./1e3]);
set(gca,'YDir', 'reverse');
ax = gca; %
ax.Box = 'off';
ax.XAxisLocation = 'top'; %
ax.YAxisLocation = 'left'; %
text(0.1,0.72,{['Data Source:CCHDO'],['Longitude:',num2str(lon(n)),'°E'],['Latitude:',num2str(lat(n)),'°N'],...
...
xlabel(['Gradient (µmol/L/m)']);
xlim([-0.025 0.06]);
ylim([-0.01 max(odepth)./1e3]);
% ylabel('Depth (km)');
set(gca,'YDir', 'reverse');
ax = gca; %
ax.Box = 'off';
